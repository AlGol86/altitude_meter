...
void init_ssd1306_2(char adr) //init for oled128*64
{
   ...
   sendCmd(adr,0x3F); //0x3F for 128*64 0x1f for 128*32
   ...
   sendCmd(adr,0x12); //0x12 for 128*64 0x02 for 128*32
...
void oled_print_mega_digit(char d,char x,char y)
{
  ...
  char dig[8];
  for (char i=0;i<8;i++) dig[i]=const_dig[d][i]; 
  
  set_cursor(oled_32_128_adr,x,y );
  for (char i=0;i<4;i++) send_repeat_data(oled_32_128_adr,dig[i],c);
  set_cursor(oled_32_128_adr,x,y+1);
  for (char i=4;i<8;i++) send_repeat_data(oled_32_128_adr,dig[i],c);
}
...
char oled_print_mega_char(char c,char x,char y)
{
  ...
  for (char i=0;i<8;i++) ch[i]=const_ch[c][i]; 
  
  set_cursor(oled_32_128_adr,x,y );
  for (char i=0;i<width_limit;i++) send_repeat_data(oled_32_128_adr,ch[i],cc);
  send_repeat_data(oled_32_128_adr,0x00,cc);
  set_cursor(oled_32_128_adr,x,y+1);
  for (char i=4;i<(4+width_limit);i++) send_repeat_data(oled_32_128_adr,ch[i],cc);
  send_repeat_data(oled_32_128_adr,0x00,cc);
...
char oled_print_graphic(char* data,char n,char x,char width)
{
  ...
  case 1: 
    {
      for (char i=0;i<n;i++) if(*(data+i)<32) asm("nop"); 
   ...
   case 2: 
     {
      for (char i=0;i<n;i++) if(*(data+i)<31) asm("nop"); 
         else if(max_data<(*(data+i))){max_data=*(data+i);}
      scale=(max_data/31)+1;
